...
# https://github.com/whitequark/parser/blob/master/test/test_parser.rb
#
# However, rather than using them as tests, we stub the test methods
# to write the tests we actually want to run.  That is, each source
# string is compiled to AST using the rubinius runtime parser and sent
# to stdout surrounded by the test apparatus we use in the other specs.
# The idea is that we can just pipe the output of this script to a file,
# then run that file as the tests.  The generated file is tracked in
...
(TestParser.new) if \
class TestParser
  # Use the rbx compiler to convert the source to an AST
  def source_to_ast source
    compiler = Rubinius::ToolSets::Runtime::Compiler.new :string, :ast
    compiler.parser.root Rubinius::ToolSets::Runtime::AST::Script
    compiler.parser.input source, '(snippet)', 1
    compiler.run
    compiler.parser.instance_variable_get(:@output).body
  end
  
  # Output an rbx-style assertion for the assertion
  # Expressions that cause rbx errors are commented out and marked with "ERROR"
  def assert_parses ast, source, *rest
    begin
      result = source_to_ast source
      result = PP.pp result.to_sexp, ""
      result = result.split("\n").join("\n    ")
      puts "  # #{@this_meth}"
      puts "  parse #{source.inspect} do"
      puts "    "+result
      puts "  end"
      puts "  "
    rescue Exception
      puts "  # # ERROR in: #{@this_meth}"
      puts "  # parse #{source.inspect}"
      puts "  "
    end
  end
  
  # NOOP the following methods and constants
  def s *args; end
  def assert_diagnoses *args; end
  def refute_diagnoses *args; end
  def with_versions *args; end
  ALL_VERSIONS = []
  
  # Run each test method when instantiated
  def initialize
    puts 'describe "An AST node" do'
